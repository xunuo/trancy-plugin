(function() {
     ...
     * - 返回单词列表页面时候回到之前滚动的位置
     * - 增加当前生词导出
     * - 偏移视频播放主界面避免侧边栏挡住视频或字幕
     * - 视频跳转播放片段后关闭单词详情页回到之前播放进度*
     **/ 
    ...
    parentElement.addEventListener('click', (event) => {
        ...
        // 点击侧边栏关闭按钮返回播放进度（无论是单词列表还是单词详情）
        if (event.target.parentElement.parentElement.classList.contains('icon-back') || event.target.parentElement.classList.contains('icon-back') || event.target.classList.contains('icon-back')) {
            // 回到播放备份进度
            if(backupVideoTime){
                JumpToTime(backupVideoTime);
            }
        }
